...
. contains some custom scripts
    ...
    - /usr/local/catalina-base/bin/start.sh custom start up script. Arranging gracefuil shutdown. 
    - /usr/local/catalina-base/bin/setenv.sh  picked up by tomcat
    - /usr/local/catalina-base/bin/jmx.sh inclued by setenv.sh, but can also be used remotely for easy figuring out that relevant ports to forward are
